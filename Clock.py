...
try:
    ...
    user_config = {}
# Default config values (for any missing keys)
default_config = {
    "window_width": 300,
    "window_height": 300,
    "digital_clock": True,
...
class RomanClock:
    def __init__(self, root,config):
        ...
        #self.root.wm_attributes("-alpha", config["opacity"])
        self.root.resizable(False, False)
        self.root.bind("<Escape>", lambda e: self.root.destroy())
        
        # rest of your initialization code here,
        # use config["foreground_color"], config["font_name"], config["font_size"], etc.
         
          # Example using digital_clock config
        if config["digital_clock"]:
            self.label = tk.Label(root, text="", fg=config["foreground_color"], bg=config["background_color"], font=("Consolas", config["font_size"]))
            self.label.pack()
        else:
            self.label = None

        # Handle follow_mouse config
        #self.follow_mouse = config["follow_mouse"]
        self.follow_mouse = config.get("follow_mouse", False)


        # Show exit button only if config allows
        if config["show_exit_button"]:
            self.exit_btn = tk.Button(root, text="X", font=("Arial", 7), fg="red", bg=config["background_color"],
...
if __name__ == "__main__":
    # Load all config settings from config.txt
    config = load_config()
    ...
    # Pass the config dictionary to your clock class (you'll need to modify RomanClock to accept config)
